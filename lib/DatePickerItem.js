...
class DatePickerItem extends Component<void, Props, State> {
    ...
    _iniDates(date) {
        ...
        const dates = Array(...Array(DATE_LENGTH))
            .map((value, index) =>
                TimeUtil[`next${typeName}`](date, (index - MIDDLE_INDEX) * this.props.step, this.props.relative));
        this.setState({ dates });
    }
    ...
    _updateDates(direction) {
        ...
        if (direction === 1) {
            ...
            this.setState({
                dates: [
                    ...dates.slice(1),
                    TimeUtil[`next${typeName}`](dates[dates.length - 1], this.props.step, this.props.relative),
                ],
                marginTop: (this.currentIndex - MIDDLE_INDEX) * DATE_HEIGHT,
            });
        } else {
            ...
            this.setState({
                dates: [
                    TimeUtil[`next${typeName}`](dates[0], -this.props.step, this.props.relative),
